...
public class UserWebtoonMapsService {
	...
	public List<WebtoonVO> saveWebtoon(long CurruntUser_facebookID,
		...
		List<WebtoonVO> webtoon_ids = this.getReadWebtoons(CurruntUser_facebookID);
			...
			if (webtoon_ids.isEmpty()) {
				...
				while (webtoonIter.hasNext()) {
					if (webtoons_id == webtoonIter.next().getWebtoons_id_pk()) {
						...
						this.updateReadWebtoon(webtoons_id,
								user_webtoon_rate, CurruntUser_facebookID);
					} else {
						System.out.println("새로운 웹툰 추가 repetitionRemoval 실행 ");
						this.addReadWebtoon(webtoons_id,
								user_webtoon_rate, CurruntUser_facebookID);
					}
				}
			}
	...
	public String doInsertReserveWebtoon(long users_facebookID_fk, int webtoons_id) {
...
//			return "Error!";
	...
	public boolean addReserveWebtoon(long users_facebookID_fk, int webtoons_id_fk) {
		...
		User_Webtoon_MapsDAO userWebtoonMapsDAO = mysqlDAOFactory.getUser_Webtoon_MapsDAO();
		
		return userWebtoonMapsDAO.addReserveWebtoon(users_facebookID_fk, webtoons_id_fk);
	}
	
	//2014.07.14 soo 구조 변경 (첫 읽은 웹툰 별점 추가 - DAO 빼기)
	public void addFirstReadWebtoon(int webtoons_id, int user_webtoon_rate, long users_facebookID_fk) {
		MySqlDAOFactory mysqlDAOFactory = new MySqlDAOFactory();
		User_Webtoon_MapsDAO userWebtoonMapsDAO = mysqlDAOFactory.getUser_Webtoon_MapsDAO();
		
		userWebtoonMapsDAO.addFirstReadWebtoon(webtoons_id, user_webtoon_rate, users_facebookID_fk);
	}
	
	// 2014.07.14 soo 구조 변경 (읽은 웹툰 별점 추가 - DAO 빼기)
	public void addReadWebtoon(int webtoons_id, int user_webtoon_rate, long users_facebookID_fk) {
		MySqlDAOFactory mysqlDAOFactory = new MySqlDAOFactory();
		User_Webtoon_MapsDAO userWebtoonMapsDAO = mysqlDAOFactory.getUser_Webtoon_MapsDAO();
		
		userWebtoonMapsDAO.addReadWebtoon(webtoons_id, user_webtoon_rate, users_facebookID_fk);
	}
	...
	public void updateReadWebtoon(int webtoons_id, int user_webtoon_rate, long users_facebookID_fk) {
		MySqlDAOFactory mysqlDAOFactory = new MySqlDAOFactory();
		User_Webtoon_MapsDAO userWebtoonMapsDAO = mysqlDAOFactory.getUser_Webtoon_MapsDAO();
		
		userWebtoonMapsDAO.updateReadWebtoon(webtoons_id, user_webtoon_rate, users_facebookID_fk);
	}
	...
	public void calculateAverageRate(int webtoons_id, int user_webtoon_rate) {
		...
		double averageRate = (double)averageRateVO.getSumRate() / averageRateVO.getCountUsers();
